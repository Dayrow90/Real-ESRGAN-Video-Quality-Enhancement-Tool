...
class VideoEnhancerTaskBase:
    ...
    def create_widgets(self, title):
        ...
        tk.Entry(file_frame, textvariable=self.video_path_var, state="readonly").pack(
            side=tk.LEFT, fill=tk.X, expand=True
        )

        tk.Button(file_frame, text="浏览", command=self.browse_video).pack(
        ...
        tk.Entry(file_frame, textvariable=self.video_out_var, state="readonly").pack(
            side=tk.LEFT, fill=tk.X, expand=True
        )

        tk.Button(file_frame, text="浏览", command=self.browse_video_out).pack(
            side=tk.RIGHT, padx=(5, 0)
        )
    ...
    def on_enter_cut_head_label(self, *args):
        ...
        output_image_path = os.path.join(
            self.parent.dir_capture, f"{basename}_head_{sec}.png"
        )
    ...
    def on_enter_cut_tail_label(self, *args):
        ...
        basename = os.path.basename(path)
        output_image_path = os.path.join(
            self.parent.dir_capture, f"{basename}_tail_{sec}.png"
        )
        self.capture_frame(path, output_image_path, -sec)
        if not os.path.isfile(output_image_path):
            return
